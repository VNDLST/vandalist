...
library (different drive letters per machine, same underlying cloud
library). That sync client runs independently of git and cannot represent
filesystem symlinks — this is why `CLAUDE.md` is a real file with full
content (not a symlink to `AGENTS.md` as it used to be) and `AGENTS.md` is
now the thin pointer instead. If you ever need to update `AGENTS.md` for a
tool that specifically reads that filename, keep it as a short pointer to
this file, not a symlink and not a second full copy that can drift.

**Health check worth running at the start of a session if the repo has
just synced from another machine:** `git status --short` — flag anything
named `*-v1.*`, `*-v2.*`, `filename (1)`, etc. (SharePoint conflict
copies, safe to delete once confirmed identical to the real file, but
compare first). If `CLAUDE.md` is ever unexpectedly empty or tiny, that's
sync damage — restore from git (`git restore CLAUDE.md`) rather than
